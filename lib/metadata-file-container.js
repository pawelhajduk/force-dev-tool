...
var Metadata = require('metadata-lib').Metadata;
...
MetadataFileContainer.prototype.parse = function() {
	...
	var parsedContent = Metadata.parse(self.contents).toJSON();
	var type = self.getMetadataType();
	var objectName = self.getComponent().fullName;
	var renderOpts = self.opts.ignoreWhitespace ? {
		pretty: false
	} : null;
	if (type.childXmlNames) {
		type.childXmlNames.forEach(function(childXmlName) {
			...
			if (parsedContent[childType.tagName]) {
				var children = Array.isArray(parsedContent[childType.tagName]) ? parsedContent[childType.tagName] : [parsedContent[childType.tagName]];
				children.forEach(function(field) {
					var childMember = field[childType.key];
					if (childMember) {
						var childComponent = {};
						childComponent[childType.tagName] = field;
						self.components.push(new MetadataFileComponent({
							fullName: childType.notCustomObjectRelated ? childMember : objectName + '.' + childMember,
							type: childTypeName,
							contents: new Metadata(childComponent).toString(true, renderOpts),
							value: field,
							parent: type
						}));
					}
...
MetadataFileContainer.prototype.compose = function() {
	var self = this;
	var componentValues = self.components.map(function(c) {
		return Metadata.parse(c.contents.toString());
	});
	var obj = _.groupBy(componentValues, function(c) {
		return c.getType();
	});
	Object.keys(obj).forEach(function(tagName) {
		obj[tagName] = obj[tagName].map(function(c) {
			return c.getContent();
		});
		obj[tagName] = obj[tagName].sort(function(a, b) {
			return MetadataUtils.compareMetadataFullNames(a.fullName, b.fullName);
		});
		// console.log('objtag', obj[tagName]);
		// obj[tagName] = obj[tagName].forEach(function(cmp) {
		// 	return cmp.value;
		// });
	});
	return obj;
...
MetadataFileContainer.prototype.toString = function() {
	var self = this;
	var type = self.getMetadataType();
	var composed = self.compose();
	var object = {};
	object[type.xmlName] = composed;
	return new Metadata(object).toString();
};
...
// STATIC
var builtInProps = ['diff', 'getManifest', 'parse', 'addComponent', 'getGroupedAndSortedComponents', 'compose', 'writeContents', 'toString', 'diffMaps'];
