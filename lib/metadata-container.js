...
MetadataContainer.filterStream = function(opts) {
	var container;

	return miss.through.obj(function(metadataContainer, end, cb) {
		container = metadataContainer;
		cb(null, metadataContainer);
	}, function(cb) {
		if (container && opts.excludeIgnored) {
			container.manifest.manifestJSON = Manifest.filterExcludedMetadata(container.manifest.manifestJSON, opts.ignorePatterns);
			container.destructiveManifest.manifestJSON = Manifest.filterExcludedMetadata(container.destructiveManifest.manifestJSON, opts.ignorePatterns);
		}
		cb(null, container);
	});
}
...
MetadataContainer.outputStream = function(opts) {
	...
	return miss.through.obj(function(metadataContainer, enc, cb) {
		...
		self.push(
			new MetadataFile({
				path: 'package.xml',
				contents: new Buffer(manifest.toPackageXml(false))
			})
		);
		if (destructiveManifest.manifest().length) {
			self.push(
				new MetadataFile({
					path: 'destructiveChanges.xml',
					contents: new Buffer(destructiveManifest.toPackageXml(true))
				})
			);
		}
		cb();
