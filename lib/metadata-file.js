...
MetadataFile.prototype.isIgnored = function(ignorePatterns) {
	var self = this;
	var metadataType = self.getMetadataType();
	var metadataFilename = self.getMetadataFilename();
	if (!metadataType) {
		return true;
	}
	var ignoredMatches = ignorePatterns.find(function(ignorePattern) {
		return ignorePattern === (metadataType.xmlName + '/*') || 
			ignorePattern === (metadataType.xmlName + '/' + metadataUtils.getFileNameWithoutExtension(metadataFilename));
	});
